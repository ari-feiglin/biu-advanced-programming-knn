...
void thread_job(Address addr, CLI cli, dubdset* dataset, DefaultIO* dio) {
...
int main(int argc, char** argv) {
    ...
    /*while (true) {
        TCPSocket client = server.accept_connection(3); // times out after 5 minutes with no connection
        if (!client.is_good()) {
    ...
    CLI(&com1, &com2, &com3, &com4, &com5, &com6).start(data_set, dio);
    delete data_set;
}
